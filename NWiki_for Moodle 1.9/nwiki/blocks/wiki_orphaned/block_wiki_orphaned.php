...
class block_wiki_orphaned extends block_base {
   ...
   var $pagesel = array();
    ...
    function init() {
        ...
        $this->title = get_string('block_orphaned', 'wiki');
    ...
    function get_content() {
    	...
    	if (count($pages)!=0){
    		...
    		foreach ($pages as $page){
    			$text.= '<tr>
    				<td nowrap="nowrap">
    					<a href="'.$dir.'&amp;gid='.$WS->groupmember->groupid.'&amp;uid='.$WS->member->id.'&amp;page='.urlencode($page).'" title="'.urlencode($page).'">'.format_text($this->trim_string($page,20),FORMAT_PLAIN).'</a>
    				</td>
    				</tr>';
    		}
    		$text.='</table>';
